...
test('isRTL covers expanded ranges', () => {
    assert.ok(core.isRTL(cp('پ')), 'Hebrew');
...
test('rtlMajority: majority script decides ambiguous mixed text', () => {
    ...
    assert.ok(!core.rtlMajority('The word سلام means hello'));
    // Hebrew sentence with an embedded English term -> RTL majority.
    assert.ok(core.rtlMajority('من یک string داخل جمله نوشتم'));
    ...
    assert.ok(core.rtlMajority('123 456 !!! سلام'));
    assert.ok(!core.rtlMajority('123 456 !!! abc'));
    assert.ok(!core.rtlMajority(''));
});

test('currency $ is NOT treated as LaTeX', () => {
    assert.deepStrictEqual(core.findLatexRanges('قیمت امروز $5.99 است'), []);
    assert.deepStrictEqual(core.findLatexRanges('قیمت از $5 تا $10 است'), []);
    assert.deepStrictEqual(core.findLatexRanges('costs $20 and $30'), []);
});

test('real LaTeX is detected', () => {
    assert.strictEqual(core.findLatexRanges('اینجا $x^2$ است').length, 1);
    assert.strictEqual(core.findLatexRanges('فرمول $$\\frac{a}{b}$$ اینجاست').length, 1);
...
test('segmentText with no math returns single text segment', () => {
    const segs = core.segmentText('متن ساده با قیمت $5');
    assert.strictEqual(segs.length, 1);
    assert.strictEqual(segs[0].type, 'text');
});

test('cellDir: contains-RTL beats first-strong (header starting with Latin term)', () => {
    assert.strictEqual(core.cellDir('blob محلی (HEAD c16c988)'), 'rtl'); // Latin-first but Hebrew column
...
test('tableDirFromCells: header majority RTL → rtl', () => {
    // The example from the discussion: فارسی | English | آوانویسی
    const headers = [core.firstStrong('فارسی'), core.firstStrong('English'), core.firstStrong('آوانویسی')];
...
test('table with Latin-first Hebrew headers flips (regression: CDN comparison table)', () => {
    ...
    const headers = ['فایل', 'blob محلی (HEAD c16c988)', 'blob از-CDN', 'نتیجه'].map(core.cellDir);
...
test('tableDirFromCells: first column tie-breaks when headers are inconclusive', () => {
    ...
    const firstCol = [core.firstStrong('سلام'), core.firstStrong('سپاس'), core.firstStrong('خانه')];
    assert.strictEqual(core.tableDirFromCells(headers, firstCol), 'rtl');
});
...
test('findMathRanges isolates bare arithmetic inside Hebrew', () => {
    assert.deepStrictEqual(mathSubs('نتیجه 2 + 3 = 5 است'), ['2 + 3 = 5']);
    assert.deepStrictEqual(mathSubs('بازه 5-3 درجه است'), ['5-3']);
    assert.deepStrictEqual(mathSubs('1/2 از کیک مانده'), ['1/2']);
...
test('findMathRanges keeps a trailing sentence period out of the island', () => {
    ...
    assert.deepStrictEqual(mathSubs('پس: 10 / 2 = 5, درست'), ['10 / 2 = 5']);
});

test('findMathRanges keeps internal decimals/thousands intact', () => {
    assert.deepStrictEqual(mathSubs('جمع 1.5 + 2.5 = 4.0 تومان'), ['1.5 + 2.5 = 4.0']);
    assert.deepStrictEqual(mathSubs('تقریباً 3.14 * 2 است'), ['3.14 * 2']);
});

test('findMathRanges leaves lone numbers / dates / IPs / versions alone', () => {
    assert.deepStrictEqual(mathSubs('من 5 کتاب دارم'), []);          // lone number
    assert.deepStrictEqual(mathSubs('صفحه 3 کتاب را ببین'), []);        // lone number
    assert.deepStrictEqual(mathSubs('نسخه اکنون 1.14271.0.0 است'), []); // dotted version, no operator
...
test('findMathRanges leaves currency and Hebrew-glued numbers alone', () => {
    assert.deepStrictEqual(mathSubs('قیمت فقط $5 + $10 است'), []);    // '$' breaks the run
    assert.deepStrictEqual(mathSubs('امروز $5.99 است'), []);        // currency, no operator
    assert.deepStrictEqual(mathSubs('روز-15 ماه دیدار کردیم'), []);      // Hebrew-glued, no whitespace boundary
});
...
test('segmentText isolates bare arithmetic as a math segment', () => {
    const segs = core.segmentText('فارسی 2 + 3 = 5 ادامه');
    assert.strictEqual(segs.length, 3);
    assert.strictEqual(segs[0].type, 'text');
    assert.strictEqual(segs[1].type, 'math');
